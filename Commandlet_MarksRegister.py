...
class MarksRegisterCommandlet:
    ...
    def authorize(self, msg):
        ...
        if session[0] == True:
            ...
            self.bot.send_message(msg.chat.id, session[1], reply_markup = registerKeyboard(), parse_mode='HTML')
            self.bot.send_message(msg.chat.id, 'Попробуешь еще раз?', reply_markup = registerKeyboard(), parse_mode='HTML')
